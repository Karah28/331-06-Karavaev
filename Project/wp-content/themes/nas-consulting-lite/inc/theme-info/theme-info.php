 ...
 */
if ( ! class_exists( 'nusaconsulting_Theme_Info' ) ) {
    class nusaconsulting_Theme_Info{
        ...
        public function __construct( $config ) {
            ...
            /* Admin menu */
            add_action( 'admin_menu', array( $this, 'bknusa_admin_menu' ) );

            /* Enqueue script and style for about page */
            add_action( 'admin_enqueue_scripts', array( $this, 'style_and_scripts' ) );

            /* Ajax callback for dismissable required actions */
            add_action( 'wp_ajax_nusaconsulting_theme_info_update_recommended_action', array( $this, 'update_recommended_action_callback' ) );
        }
        ...
        public function prepare_class() {
            ...
            $this->page_title    = $this->theme_name . esc_html__( ' Info', 'nas-consulting-lite' );
            $this->menu_title    = $this->theme_name . esc_html__( ' Theme', 'nas-consulting-lite' );
        ...
        function bknusa_admin_menu() {
            ...
            if ( ! empty( $this->page_title ) && ! empty( $this->menu_title ) ) {
                ...
                add_theme_page( $this->page_title, $menu_title, 'edit_theme_options', $this->theme_slug . '-info', array(
                    $this,
                    'nusaconsulting_theme_info_screen',
                ) );
            }
        }
         ...
         */
        public function nusaconsulting_theme_info_screen() {
            ...
            if (
                ...
                echo '<a href="' . esc_url( 'https://www.templatehouse.net/themes/business-consulting-wordpress-theme/' ).'" target="_blank">';
            ...
            if ( ! empty( $recommended_actions ) ) {
                ...
                if ( ! empty( $recommended_actions ) && is_array( $recommended_actions ) ) {
                    ...
                    foreach ( $recommended_actions as $action_key => $action_value ) {
                        ...
                        echo "<div class='bknusa-theme-info-action-recommended-box {$done}'>";

                        if ( ! $hidden ) {
                            echo '<span data-action="dismiss" class="dashicons dashicons-visibility bknusa-theme-info-recommended-action-button" id="' . esc_attr( $action_value['id'] ) . '"></span>';
                        } else {
                            echo '<span data-action="add" class="dashicons dashicons-hidden bknusa-theme-info-recommended-action-button" id="' . esc_attr( $action_value['id'] ) .'"></span>';
                        }
                        ...
                        if ( ! empty( $plugin_slug ) ) {
                            ...
                            switch ( $active['needs'] ) {
                                ...
                                case 'install':
                                    ...
                                    $label = esc_html__( 'Install', 'nas-consulting-lite' );
                                    break;

                                case 'activate':
                                    $class = 'activate-now button button-primary';
                                    $label = esc_html__( 'Activate', 'nas-consulting-lite' );

                                    break;

                                case 'deactivate':
                                    $class = 'deactivate-now button';
                                    $label = esc_html__( 'Deactivate', 'nas-consulting-lite' );
                ...
                if ( $hooray ){
                    echo '<span class="hooray">' . esc_html__( 'Hooray! There are no recommended actions for you right now.', 'nas-consulting-lite' ) . '</span>';
                    echo '<a data-action="reset" id="reset" class="reset-all" href="#">'.esc_html__('Show All Recommended Actions', 'nas-consulting-lite').'</a>';
                }
                echo '</div>';
            }
        }
        ...
        public function call_plugin_api( $slug ) {
            ...
            if ( false === ( $call_api = get_transient( 'nusaconsulting_theme_info_plugin_information_transient_' . $slug ) ) ) {
                ...
                set_transient( 'nusaconsulting_theme_info_plugin_information_transient_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
            }
        ...
        public function child_themes() {
            echo '<div id="child-themes" class="bknusa-theme-info-tab-pane">';
            ...
            if ( ! empty( $child_themes ) ) {
                ...
                if ( ! empty( $child_themes ) && is_array( $child_themes ) ) {
                    echo '<div class="bknusa-about-row">';
                    $i = 0;
                    foreach ( $child_themes as $child_theme ){
                        ...
                        if( !empty( $screenshot) ){
                            echo '<div class="bknusa-about-child-theme">';
                            echo '<div class="bknusa-theme-info-child-theme-image">';

                            echo '<img src="' . esc_url( $screenshot ) . '" alt="' . ( ! empty( $title ) ? esc_attr( $title ) : '' ) . '" />';

                            if ( ! empty( $title ) ) {
                                echo '<div class="bknusa-theme-info-child-theme-details">';
                                echo '<div class="theme-details">';
                                echo '<span class="theme-name">' . esc_html( $title  ). '</span>';
                                if ( ! empty( $download_link ) ) {
                                    echo '<a href="' . esc_url( $download_link ) . '" class="button button-primary install right">' . esc_html__( 'Download', 'nas-consulting-lite' ) . '</a>';
                                }
                                if ( ! empty( $preview_link ) ) {
                                    echo '<a class="button button-secondary preview right" target="_blank" href="' . $preview_link . '">' . esc_html__( 'Live Preview', 'nas-consulting-lite' ). '</a>';
                                }
                                echo '</div>';
                                echo '</div>';
                            }
                        ...
                        if( 0 == $i % 3 ){
                            echo "</div><div class='bknusa-about-row'>";/*.bknusa-about-row end-start*/
                        }
                    }

                    echo '</div>';/*.bknusa-about-row end*/
                }// End if().
            }// End if().
            echo '</div>';
        }
        ...
        public function free_pro() {
            ...
            if ( ! empty( $free_pro ) ) {
                ...
                if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
                    ...
                    echo '<div id="free_pro" class="bknusa-theme-info-tab-pane bknusa-theme-info-fre-pro">';
                    echo '<table class="free-pro-table table-light-wrapper bknusa-theme-info-text-center">';
                    echo '<thead>';
                    echo '<tr>';
                    echo '<th>' . esc_html__( 'Theme Features','nas-consulting-lite' ) . '</th>';
                    echo '<th>' . esc_html__( 'Free','nas-consulting-lite' ) . '</th>';
                    echo '<th>' . esc_html__( 'Pro (Premium)','nas-consulting-lite' ) . '</th>';
                    echo '</tr>';
                    echo '</thead>';
                    echo '<tbody>';
        ...
        public function style_and_scripts( $hook_suffix ) {
            ...
            wp_enqueue_style( 'bknusa-theme-info-css', get_template_directory_uri() . '/inc/theme-info/assets/css/theme-info.css' );
            ...
            if ( 'appearance_page_' . $this->theme_slug . '-info' == $hook_suffix ) {
                ...
                $count = $this->count_recommended_actions();
                wp_localize_script( 'bknusa-theme-info-js', 'nusaconsulting_theme_info_object', array(
...
$config = array(
    ...
    // Main welcome title
    'info_title' => sprintf( esc_html__( 'Welcome to %s - ', 'nas-consulting-lite' ), $theme_name_config ),

    // Main welcome content
    'info_content' => sprintf( esc_html__( '%s is now installed and ready to use. We hope the following information will help and you enjoy using it!', 'nas-consulting-lite' ), '<b>'.$theme_name_config.'</b>' ),
    ...
    'quick_links' => array(
        'theme_url'  => array(
            'text' => __( 'Theme Info', 'nas-consulting-lite' ),
            'url' => esc_url( 'https://www.templatehouse.net/themes/business-consulting-wordpress-theme/' )
        ),

    ),

    'tabs' => array(
        'getting_started'      => esc_html__( 'Getting Started', 'nas-consulting-lite' ),
        
        'recommended_plugins'  => esc_html__( 'Recommended Plugins', 'nas-consulting-lite' ),
    ...
    'gs_steps' => array(
        ...
        'two' => array(
            'title' => esc_html__( 'Pre-built Demos', 'nas-consulting-lite' ),
            'desc' => esc_html__( 'We recommend utilizing our collection of pre-designed starter site templates as an efficient and convenient way to begin your project.', 'nas-consulting-lite' ),
            'button_label' => esc_html__( 'Browse Demos', 'nas-consulting-lite' ),
            'button_link' => esc_url( 'https://demo.templatehouse.net/nasconsulting' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        ...
        'seven' => array(
            'title' => esc_html__( 'Need WordPress Theme Developemnt or Customization?', 'nas-consulting-lite' ),
            'desc' => esc_html__( 'We Create Beautiful & SEO Friendly WordPress Websites. Theme Customization to Custom WordPress Plugin Development.', 'nas-consulting-lite' ),
            'button_label' => esc_html__( 'Contact Now', 'nas-consulting-lite' ),
        ...
        'five' => array(
            'title' => esc_html__( 'Love Our Product?', 'nas-consulting-lite' ),
            'desc' => esc_html__( 'We would greatly appreciate your motivation through a 5-star rating on our profile.', 'nas-consulting-lite' ),
            'button_label' => esc_html__( 'Rate Us', 'nas-consulting-lite' ),
            'button_link' => esc_url( 'https://wordpress.org/support/theme/' .$theme_name_config_url. '/reviews' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'six' => array(
            'title' => esc_html__( 'Have Question? Ask Us, Now!', 'nas-consulting-lite' ),
            'desc' => esc_html__( 'We are committed to resolving any issues that may arise to ensure a seamless experience while using our theme.', 'nas-consulting-lite' ),
            'button_label' => esc_html__( 'Get Support', 'nas-consulting-lite' ),
...
return new nusaconsulting_Theme_Info( $config );